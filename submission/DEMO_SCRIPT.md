...
1. Open `apps/peer-ui` and show the pairing code, runtime mode, and local-only disclosure.
...
5. Save locally to prove offline-first capture, then submit to the nearby peer.
6. Switch back to `peer-ui` and narrate the stages: OCR, transcription, summary, grounded answer.
7. Open the markdown export and point to the evidence trail plus non-diagnostic disclaimer.

## Suggested spoken framing

- "MedMesh keeps the capture workflow private on the phone and moves heavier reasoning to a trusted nearby device."
- "All AI workloads are local or peer-local through QVAC, with no cloud AI APIs."
